	...
	for k,v in pairs( team.GetPlayers( TEAM_ARMY ) ) do
		...
		if dist < self.SoundRadius then
			...
			if ( v.NextRadSound or 0 ) < CurTime() then
				...
				v:NoticeOnce( translate.Get( "rd_notices_a_radioactive_zombie_is_nearby" ), GAMEMODE.Colors.Blue )
				v:NoticeOnce( translate.Get( "rd_notices_radioactive_zombies_will_poison_you" ), GAMEMODE.Colors.Blue, 3, 2 )
